...
void Cliente::comprar(ListaProductos &o){
    ...
    if (r == "si"){
        for (int i = 0; i < VecCompras.size(); i++){
            for (int e = 0; e < o.VecProductos.size(); e++){
                if(VecCompras.at(i).nombre == o.VecProductos.at(e).nombre){
                    o.VecProductos.at(e).cantidadcomprada();
                }
            }
        }
        if (visa < re){
